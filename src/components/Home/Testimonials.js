...
function ButtonGroup({ next, previous }) {
  return (
    <div style={{display: 'flex', justifyContent: 'center'}}>
        <div>
          <button onClick={previous} aria-label="Previous" className="text-5xl mb-12 mr-4">
            {"<"}
          </button>
          <button onClick={next} aria-label="Next" className="text-5xl mb-12 ml-4">
            {">"}
          </button>
        </div>
    </div>
        ...
        <Carousel {...carouselParams}>
          {data.map((item, index) => (
            ...
            >
              ...
              <div className="text-justify font-playfair">{item.description}</div>
              <div style={{textAlign: 'right', marginTop: "0.4rem"}}>{item.name}</div>
            </div>
          ))}
        </Carousel>
      </div>
